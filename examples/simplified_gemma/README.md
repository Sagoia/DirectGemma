...
Build the `simplified_gemma` project from `msvc\gemma\gemma.slnx`. The
executable is written to `msvc\gemma\build\x64\<Configuration>`. Run it with
the tokenizer, compressed weights file, and model type, for example:

```powershell
msvc\gemma\build\x64\Release\simplified_gemma.exe --tokenizer tokenizer.spm --weights 2b-it-sfp.sbs --model 2b-it
```
...
```powershell
msvc\gemma\build\x64\Release\simplified_gemma.exe [...] --reject 32338 42360 78107 106837 132832 143859 154230 190205
```
